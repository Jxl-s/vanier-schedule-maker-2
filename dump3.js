...
async function dump(clear = true) {
    ...
    async function getSecureConfig(url) {
        ...
        let viewLayouts = responseText.match(/data-view-layouts='(.+?)'/) ?? responseText.match(/data-view-layouts="(.+?)"/);
    ...
    async function getTokens() {
        ...
        const responseText = await response.text();
        const token = responseText.match(/__RequestVerificationToken" type="hidden" value="(.+?)"/)[1];

        const cookie = response.headers.get("set-cookie").match(/__RequestVerificationToken=(.+?);/)[1];
        return [cookie, token]
    }

    async function getCourses(cookie, token, page = 1, pageSize = 40) {
        const secureConfig = await getSecureConfig(BASE_URL);
        const res = await fetch(BASE_URL + "/_services/entity-grid-data.json/c7a13072-c94f-ed11-bba3-0022486daee2", {
            "headers": {
            ...
            "referrer": BASE_URL,
            "body": JSON.stringify({ base64SecureConfiguration: secureConfig, sortExpression: "vit_course ASC,vit_sec ASC", search: "", page, pageSize, pagingCookie: "", filter: null, metaFilter: null, timezoneOffset: 300, customParameters: [] }),
            "method": "POST",
        });
        ...
        for (const course of resJson.Records) {
            courses.push({
                id: course.Id,
                section: course.Attributes.find((attr) => attr.Name === "vit_sec").Value,
                courseId: course.Attributes.find((attr) => attr.Name === "vit_course").Value,
                title: course.Attributes.find((attr) => attr.Name === "vit_coursetitle").Value,
            })
        }
    ...
    async function getClasses(cookie, token, entityId) {
        const secureConfig = await getSecureConfig(`${BASE_URL}/_portal/modal-form-template-path/c7a13072-c94f-ed11-bba3-0022486daee2?id=${entityId}&entityformid=${FORM_ID}&languagecode=1033`);
        const res = await fetch(BASE_URL + "/_services/entity-grid-data.json/c7a13072-c94f-ed11-bba3-0022486daee2", {
            "headers": {
                cookie: `__RequestVerificationToken=${cookie}`,
                "Content-Type": "application/json; charset=utf-8",
        ...
        for (const record of resJson.Records) {
            const teacher = record.Attributes.find((attr) => attr.Name === "vit_teacher")?.Value;
            const day = record.Attributes.find((attr) => attr.Name === "vit_day")?.Value?.slice(0, 3);
            const time = record.Attributes.find((attr) => attr.Name === "vit_time")?.Value;
            const room = record.Attributes.find((attr) => attr.Name === "vit_room")?.Value;
    ...
    while (allCourses.length > 0) {
        ...
        fs.writeFileSync(`dump_output/${currentPage}.json`, JSON.stringify(pageDump));
